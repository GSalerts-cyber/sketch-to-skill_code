...
DATASETS = {
    "Assembly": "~/demo_dataset_bc_96_new/Assembly_frame_stack_1_96x96_end_on_success/dataset.hdf5",
    "BoxClose": "~/demo_dataset_bc_96_new/BoxClose_frame_stack_1_96x96_end_on_success/dataset.hdf5",
    "StickPull": "~/raw_demo_servoring_data/StickPull/StickPull_teachermodel_gen_10.hdf5",
    "CoffeePush": "~/demo_dataset_bc_96_new/CoffeePush_frame_stack_1_96x96_end_on_success/dataset.hdf5",
    # "ButtonPress": "~/demo_dataset_bc_96_new/ButtonPress_frame_stack_1_96x96_end_on_success/dataset.hdf5",
    # "ButtonPressWall": "~/demo_dataset_bc_96_new/ButtonPressWall_frame_stack_1_96x96_end_on_success/dataset.hdf5",
    # "DrawerOpen": "~/demo_dataset_bc_96_new/DrawerOpen_frame_stack_1_96x96_end_on_success/dataset.hdf5",
    # "Reach": "~/demo_dataset_bc_96_new/Reach_frame_stack_1_96x96_end_on_success/dataset.hdf5",
    # "CofeeButton": "~/demo_dataset_bc_96_new/CoffeeButton_frame_stack_1_96x96_end_on_success/dataset.hdf5",
    # "ReachWall": "~/demo_dataset_bc_96_new/ReachWall_frame_stack_1_96x96_end_on_success/dataset.hdf5",
    # "ButtonPressTopdownWall": "~/demo_dataset_bc_96_new/ButtonPressTopdownWall_frame_stack_1_96x96_end_on_success/dataset.hdf5",

####Sketch##################

    "ButtonPress": "~/raw_demo_servoring_data/ButtonPress/ButtonPress_teachermodel_gen_10.hdf5",
    "DrawerOpen": "~/raw_demo_servoring_data/DrawerOpen/DrawerOpen_teachermodel_gen_10.hdf5",
    "Reach": "~/raw_demo_servoring_data/Reach/Reach_teachermodel_gen_10.hdf5",
    "CofeeButton": "~/raw_demo_servoring_data/CoofeeButton/CofeeButton_teachermodel_gen_10.hdf5",
    "ReachWall": "~/raw_demo_servoring_data/ReachWall/ReachWall_teachermodel_gen_10.hdf5",
    "ButtonPressTopdownWall": "~/raw_demo_servoring_data/ButtonPressTopdownWall/ButtonPressTopdownWall_teachermodel_gen_10.hdf5",
}
...
class DatasetConfig:
    ...
    frame_stack: int = 1
    action_repeat: int = 2
    task_name: str = ""     # Extra field to store task name

    def __post_init__(self):
        self.task_name = self.path
...
class MetaWorldDataset:
    def __init__(self, cfg: DatasetConfig):
        ...
        for episode_id in range(num_episode):
            ...
            episode_tag = list(f["data"].keys())[episode_id]
            
            episode = f[f"data/{episode_tag}"]
            # actions: np.ndarray = episode["actions"]  # type: ignore
            
            actions: np.ndarray = episode["actions"][:, :3]  # Only consider first 3 columns of actions - RL Sketch
            # Add extra column with zeros for actions
            ...
            robot_locs.append(episode["obs"]["prop"][:, :3])  # proprioception states, considering only first 3 columns of props
            # add extra column with zeros for props
            robot_locs.append(np.zeros((robot_locs[0].shape[0], 1)))
        ...
        # Turn on/off Random object pose -- Rl sketch
        self.env.obj_rand_init(rand_init=True)
        # self.env.set_obj_pose_sktchRL(np.array([0.000, 0.800, 0.115]))  # Set object pose 
...
def run(cfg: MainConfig, policy):
    ...
    for epoch in range(cfg.num_epoch):
        ...
        with stopwatch.time("eval"):
            ...
            scores = run_eval(dataset.env, policy, num_game=cfg.num_eval, seed=seed, verbose=False)
            if ema_policy is not None:
                ema_scores = run_eval(
                    dataset.env, ema_policy, num_game=cfg.num_eval, seed=seed, verbose=False
                )
...
def load_model(weight_file, device):
    ...
    # policy = BcPolicy(env.observation_shape, (3, 1024), 3, cfg.policy)
...
if __name__ == "__main__":
    ...
    cfg.save_dir =f"~/BC_IBRL_96_demo3/{cfg.dataset.task_name}_epoch{cfg.num_epoch}_seed{cfg.seed}"
