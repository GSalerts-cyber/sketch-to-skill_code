...
def add_demos_to_replay(
    ...
    for episode_id in range(num_episode):
        ...
        # actions: np.ndarray = np.array(episode["actions"])[:, :3]  # RL-Sketch Update
        images: np.ndarray = np.array(episode[f"obs/{rl_camera}_image"])  # type: ignore
        # np.array_equal(np.array(episode["states"]), np.array(episode["obs/state"]))
        states: np.ndarray = np.array(episode["obs/state"])  # type: ignore
        props: np.ndarray = np.array(episode["obs/prop"])  # type: ignore
        rewards: np.ndarray = np.array(f[f"data/{episode_tag}/rewards"])  # type: ignore
        # terminals = rewards
        dones = np.array(f[f"data/{episode_tag}/dones"])  # type: ignore
        terminals = dones
        ...
        for i in range(episode_len + 1):
            if i < episode_len:
                obs = {
                    ...
                    "prop": torch.from_numpy(props[i]).float(),
    ...
    print(f"Lets confirm num_success from preload: {replay.num_success}")
