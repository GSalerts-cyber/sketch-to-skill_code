...
import time
def visualize_results(original_points, model_points, img_name):
    ...
    ax1.plot(original_points[:, 0], original_points[:, 1], original_points[:, 2], 'b-*', linewidth=2)
    ...
    ax2.plot(model_points[:, 0], model_points[:, 1], model_points[:, 2], 'r-*', linewidth=2)
    ax2.set_title('Model Trajectory')
...
def save_hdf5(original_points, model_points, prefix, obj_pose, last_reached):
    with h5py.File(f"{save_file}.hdf5", 'a') as f:
        ...
        dones = np.zeros(len(model_points))
        dones[-1] = 1
        ...
        print("--------------------------------")
    print(f"Successfully Saved {save_file}.hdf5")
...
def main(original_points, model_points, img_name, obj_pose):
    ...
    visualize_results(original_points, model_points, img_name)
    # save_hdf5(original_points, fitted_trajectory, noisy_trajectories, img_name, obj_pose, last_reached)
    save_hdf5(original_points, model_points, img_name, obj_pose, last_reached)
...
if __name__ == "__main__":
    ...
    file = "~/demo_dataset_bc_96_new/ButtonPress_frame_stack_1_96x96_end_on_success/dataset.hdf5"
    d = h5py.File(file, 'r')
    ...
    #    points = demo["obs/prop"][:, :3]
        model_points = demo[0, :, :3]

        load_obj_pose = d[f"data/{key}/obs/state"][0, -3:]    
        obj_pose = load_obj_pose    
        original_points = d[f"data/{key}/obs/prop"][:, :3]
        # obj_pose = None
        ...
        # all_points.append(model_points)
        # main(points, image_name, key)
        main(original_points, model_points, key, obj_pose)
