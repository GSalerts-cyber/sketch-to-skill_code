...
project = f"VAE_MLP_Both_NewData2"
...
parameters = {
    ...
    "lr": [1e-3],
    "bs": [256, 128],
    "M_N": [0.0001, 0.00025, 0.0005, 0.0010, 0.1000],
...
for combo in combinations:
    ...
    job_name = f"train_both_bs{param_dict['bs']}_dim{param_dict['latent_dim']}_{param_dict['scheduler']}_lr{param_dict['lr']}_kld{param_dict['M_N']}"
    job_name += "_Anneal" if param_dict['kld_anneal'] else ""

    # Construct the command to run the Python script with arguments
    python_command = (
        f"python main_both.py --num_epochs {param_dict['num_epochs']} "
        f"--project {project} "
