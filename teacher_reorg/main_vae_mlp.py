...
def main(args, root_dir=None):
    ...
    model = VAE_MLP(img_size=args.img_size, in_channels=3, latent_dim=args.latent_dim, num_control_points=args.num_control_points, degree=args.degree, use_traj_rescale=args.use_traj_rescale).cuda()
    ...
    for key in test_loaders.keys():
        _, _, test_loader = get_dataloader(batch_size=args.bs, img_size=args.img_size, val_split=0, test_split=1, data_path="test", data_name=key, logger=logger)
...
if __name__ == "__main__":    
    ...
    if args.load_pretrained_vae:
        args.pretrained_vae_path = f"~/VAE_Aug_Standardize_NewData/{args.pretrained_vae_path}"
        print(f"pretrained vae path: {args.pretrained_vae_path}")
    
    unique_token = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    unique_name = f"vae_mlp_ep{args.num_epochs}_bs{args.bs}_dim{args.latent_dim}_{args.scheduler}_lr{args.lr:.5f}_kld{args.M_N:.5f}_aug"
    if args.load_pretrained_vae:
