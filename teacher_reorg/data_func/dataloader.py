...
def get_dataloader(img_size=64, batch_size=32, num_control_points=20, num_samples=None, val_split=0.2, test_split=0.1, num_workers=8, data_path="new", data_name=None, logger=None):
    # Load the data
    if data_path == "old":
        root_path = '~/sketch_datasets/'
    ...
    for f_name in file_names:
        ...
        trajectories_tensor.append(torch.load(f'{root_path}/{f_name}/{f_name}_fitted_trajectories_50.pt'))
        params_tensor.append(torch.load(f'{root_path}/{f_name}/{f_name}_params_{num_control_points}.pt'))
        fitted_trajectories_tensor.append(torch.load(f'{root_path}/{f_name}/{f_name}_fitted_trajectories_20.pt'))
    ...
    transform_traj = AugTrajectory(noise_scale=0.015, num_control_points=20, smoothness=0.05, p=0.6)
...
if __name__ == '__main__':
    ...
    file_names = ["assembly", "boxclose", "coffeepush", "ButtonPress"]
    f_name = file_names[0]
    trajectories = torch.load(f'{root_path}/{f_name}_fitted_trajectories_50.pt')

    num_subplot = 2
    num_samples = 6
    num_random = 5
    fig = plt.figure(figsize=(3*num_samples, 3*num_subplot))
    aug_traj = AugTrajectory(noise_scale=0.015, num_control_points=20, smoothness=0.05, p=1)
    ...
    plt.tight_layout()
    plt.savefig("augmented_trajectories.png")
    plt.close()
