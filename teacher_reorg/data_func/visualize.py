...
def sample_and_visualize(f_name, squential=True):
    ...
    if os.path.exists(f'{root_path}/{f_name}_rgbs1_64.pt'):
        rgbs1 = torch.load(f'{root_path}/{f_name}_rgbs1_64.pt')
        rgbs2 = torch.load(f'{root_path}/{f_name}_rgbs2_64.pt')
        rgbs1 = rgbs1[idx].permute(0, 2, 3, 1).to(int).numpy()
        rgbs2 = rgbs2[idx].permute(0, 2, 3, 1).to(int).numpy()
    else:
        rgbs1, rgbs2 = None, None

    img_name = f"{root_path}/{f_name}_sample"
