        ...
        for i, batch in enumerate(test_loader):
            ...
            if interpolate:
                ...
                for j in range(num_samples):
                    ...
                    sketch_gt_2 = [sel_sketches2[j], sel_sketches2[-j-1]]
    ...
    for i in range(num_samples):
        ...
        if trajectories is None and fitted_trajectories is None:
            ...
            for gen_traj in generated_trajectories:
                ax5.plot(gen_traj[i][:, 0], gen_traj[i][:, 1], gen_traj[i][:, 2])
            ax5.set_xlabel('X')
            ax5.set_ylabel('Y')
            ax5.set_zlabel('Z')
            ax5.set_title(f"Sample {i + 1}: Generated Trajectory")
            ax5.set_xlim(x_lim[0], x_lim[1])
            ax5.set_ylim(y_lim[0], y_lim[1])
            ax5.set_zlim(z_lim[0], z_lim[1])
            ax5.set_box_aspect((np.ptp(ax5.get_xlim()), np.ptp(ax5.get_ylim()), np.ptp(ax5.get_zlim())))
            # ax5.view_init(elev=20, azim=45)  # Adjust the view angle

        else:
            ax5 = fig.add_subplot(rows, num_samples, 4 * num_samples + i + 1, projection='3d')
            ax5.plot(fitted_trajectories[i][:, 0], fitted_trajectories[i][:, 1], fitted_trajectories[i][:, 2], 'b-', alpha=0.5, linewidth=1)
            ax5.scatter(trajectories[i][:, 0], trajectories[i][:, 1], trajectories[i][:, 2], c=np.arange(len(trajectories[i])), cmap=cmap, s=10)
            ax5.set_xlabel('X')
            ax5.set_ylabel('Y')
            ax5.set_zlabel('Z')
            ...
            ax6.scatter(trajectories[i][:, 0], trajectories[i][:, 1], trajectories[i][:, 2], c=np.arange(len(trajectories[i])), cmap=cmap, s=10)
            for gen_traj in generated_trajectories:
                ax6.plot(gen_traj[i][:, 0], gen_traj[i][:, 1], gen_traj[i][:, 2])
            ax6.set_xlabel('X')
            ax6.set_ylabel('Y')
            ax6.set_zlabel('Z')
            mse_loss = np.mean([np.mean((fitted_trajectories[i] - gen_traj[i])**2) for gen_traj in generated_trajectories])
            ax6.set_title(f"Generated Traj MSE: {mse_loss:.6f}")
            ax6.set_xlim(x_lim[0], x_lim[1])
            ax6.set_ylim(y_lim[0], y_lim[1])
            ax6.set_zlim(z_lim[0], z_lim[1])
            ax6.set_box_aspect((np.ptp(ax6.get_xlim()), np.ptp(ax6.get_ylim()), np.ptp(ax6.get_zlim())))
