...
def visualize_results(original_points, model_points, servoring_points, gripper_init_pose, obj_pose, img_name, save_data_path):
    ...
    ax11.plot(obj_pose[0], obj_pose[1], obj_pose[2] - 0.05, 'r*', markersize=20)
    ...
    ax12.plot(obj_pose[0], obj_pose[1], obj_pose[2] - 0.05, 'r*', markersize=20)
    ...
    # apply offset to obj_pose
    ax21.plot(obj_pose[0], obj_pose[1], obj_pose[2] - 0.05, 'r*', markersize=20)
    ...
    ax22.plot(obj_pose[0], obj_pose[1], obj_pose[2] - 0.05, 'r*', markersize=20)
    ...
    if servoring_points is not None:
        ...
        ax13.plot(obj_pose[0], obj_pose[1], obj_pose[2] - 0.05, 'r*', markersize=20)
        ...
        ax23.plot(obj_pose[0], obj_pose[1], obj_pose[2] - 0.05, 'r*', markersize=20)
...
def save_hdf5(env_name, model_points, demo_name, obj_pose, last_reached, save_file):
    ...
    with h5py.File(f"{save_file}.hdf5", 'a') as f:
        ...
        num_points = 20
        idx = np.linspace(0, len(model_points) - 1, num=num_points, dtype=int)
    ...
    print(f"gripper_init_pose: {states[0][:3]}, goal_pose: {states[0][-3:]}")
    print(f"offset: {obj_pose - states[0][-3:]}")
    print("--------------------------------")
...
def main(env_name, demo_name, original_points, model_points, gripper_init_pose, obj_pose, save_data_path, num_gen_per_demo, raw_demo_id):
    visualize_results(original_points, model_points, None, gripper_init_pose, obj_pose, f"{env_name}_{demo_name}", f"{save_data_path}_gen_{num_gen_per_demo}")
    ...
    for i in range(num_gen_per_demo):
        ...
        cur_servoring_points = save_hdf5(env_name, model_points[i], name, obj_pose, last_reached, f"{save_data_path}_gen_{num_gen_per_demo}")
        servoring_points.append(cur_servoring_points)
    visualize_results(original_points, model_points[:num_gen_per_demo], servoring_points, gripper_init_pose, obj_pose, f"{env_name}_{demo_name}_servo", f"{save_data_path}_gen_{num_gen_per_demo}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--env_name', type=str, default="ButtonPress", help="Save file")
    parser.add_argument('--num_gen_per_demo', type=int, default=10, help="Number of generated trajectories per raw demo")
    ...
    generated_data_paths = {
        "ButtonPress": "~/VAE_MLP_Both_NewData2_0927_rescale/vae_mlp_ep200_bs128_dim32_cosine_lr0.00100_kld0.00010_aug_2024-09-27_06-44-04/generated_trajectory_ButtonPress_inference_hand_drawn.hdf5",
    }

    goal_pose_offset = {
        "ButtonPress": np.array([0, 0.1, 0.0]),
    }

    raw_demo_data_path = raw_demo_data_paths[args.env_name]
    generated_data_path = generated_data_paths[args.env_name]

    save_path = "~/raw_demo_servoring_data"
    ...
    with h5py.File(raw_demo_data_path, 'r') as f_raw, h5py.File(generated_data_path, 'r') as f_gen:
        ...
        for raw_demo_id, demo_name in enumerate(keys):
            ...
            main(args.env_name, demo_name, original_points, model_points, gripper_init_pose, goal_pose + goal_pose_offset[args.env_name], save_data_path, num_gen_per_demo, raw_demo_id)
