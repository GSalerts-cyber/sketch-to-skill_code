...
GOOD_CAMERAS = {
    ...
    "DrawerOpen": ["corner2"],
    "ButtonPress": ["corner2"],
    "ButtonPressWall": ["corner2"],
    "CoffeeButton": ["corner2"],
    "ButtonPressTopdownWall": ["corner2"],
    "ReachWall": ["corner2"],
}
...
STATE_IDXS = {
    ...
    "Reach": list(range(39)),
    "DrawerOpen": list(range(39)),
    "ButtonPress": list(range(39)),
    "ButtonPressWall": list(range(39)),
    "CoffeeButton": list(range(39)),
    "ButtonPressTopdownWall": list(range(39)),
    "ReachWall": list(range(39)),
}
...
PROP_IDXS = {
    ...
    "Soccer": list(range(4)),
    "Reach": list(range(4)),
    "DrawerOpen": list(range(4)),
    "ButtonPress": list(range(4)),
    "ButtonPressWall": list(range(4)),
    "CoffeeButton": list(range(4)),
    "ButtonPressTopdownWall": list(range(4)),
    "ReachWall": list(range(4)),
}
...
class MetaWorldEnv(gym.Env):
    ...
    def __init__(
        ...
        # == Change the object position - Rl Sketch ==
        # self.env.random_init = False
        # self.env.init_config['obj_init_pos'] = np.array([0.000, 0.800, 0.115])
        # self.env.init_config['obj_init_pos'] = np.array([0.009, 0.771, 0.115]) - np.array([.0, -.193, .0])  # subtracted becos this is in the code
    ...
    def set_obj_pose_sktchRL(self, obj_pose):
        print(f"Setting Object Init Position: {obj_pose}")
        self.env.init_config['obj_init_pos'] = obj_pose
        # |-- offset [not-needed] --|
        # self.env.init_config['obj_init_pos'] = obj_pose - np.array([.0, -.193, .0]) # stupid offset is present in metworld library [get_pos_objects() function]
        self.env.reset_model()
        # print("obj pose: ", self.env._get_pos_objects())


    def obj_rand_init(self, rand_init):
        print(f"Randomizing Object Init Position: {rand_init}")
        self.env.random_init = rand_init
        self.env.reset_model()

    def get_obj_pose_sktchRL(self):
        return self.env._get_pos_objects()
...
class PixelMetaWorld:
    ...
    def set_obj_pose_sktchRL(self, obj_pose):
        self.env.env.set_obj_pose_sktchRL(obj_pose)


    def obj_rand_init(self, rand_init):
        self.env.env.obj_rand_init(rand_init)

    def get_obj_pose_sktchRL(self):
        return self.env.env.get_obj_pose_sktchRL()
