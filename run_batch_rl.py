...
root_dir = "~/IBRL_teachermodel_demo3_gen_1"
...
train_script_path = "~/Teacher_student_RLsketch/mw_main"

# Define environments and seeds
environments = ["Reach", "ReachWall", "ButtonPressTopdownWall","DrawerOpen", "CoffeePush"]
# environments = ["CoffeeButton"]
seeds = [0, 1, 2, 3, 4]
...
for env in environments:
    for seed in seeds:
        ...
        python_command = (
            f"python train_rl_mw.py "
            f"--config_path ~/Teacher_student_RLsketch/release/cfgs/metaworld/ibrl_basic.yaml "
            f"--bc_policy {env} "
            f"--seed {seed}"
        )
